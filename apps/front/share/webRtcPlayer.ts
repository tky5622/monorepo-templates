...
export default function (parOptions: Config) {
  ...
  oniceconnectionstatechange = function (state: {
    srcElement: { iceConnectionState: any }
  }) {
    console.info(
  ...
  onicegatheringstatechange = function (state: {
    srcElement: { iceGatheringState: any }
  }) {
    console.info(
  ...
  createDataChannel = function (
    pc: { createDataChannel: (arg0: any, arg1: any) => any },
    label: any,
    options: any
  ) {
  ...
  handleCreateOffer = function (pc: {
    createOffer: (arg0: any) => Promise<any>
    setLocalDescription: (arg0: any) => void
  }) {
  ...
  setupTransceiversAsync = async function (pc: {
    getTransceivers: () => { (): any; new (): any; length: number }
    addTransceiver: (
      arg0: string | MediaStreamTrack,
      arg1: { direction: string }
    ) => void
  }) {
  ...
  this.setVideoEnabled = function (enabled: any) {
    self.video.srcObject
      .getTracks()
      .forEach((track: { enabled: any }) => (track.enabled = enabled))
  }
  ...
  this.handleCandidateFromServer = function (
  ...
  this.receiveOffer = function (offer: RTCSessionDescriptionInit) {
    ...
    if (!self.pcClient) {
      ...
      self.pcClient.setRemoteDescription(offerDesc).then(() => {
        setupTransceiversAsync(self.pcClient).finally(function () {
          self.pcClient
            .createAnswer()
            .then((answer: any) => {
