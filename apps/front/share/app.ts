...
function gamepadDisconnectHandler(e: {
  gamepad: { id: string; index: string | number }
}) {
...
function sendStartLatencyTest() {
  ...
  const onTestStarted = function (StartTimeMs: any) {
    const descriptor = {
...
function playStream() {
  ...
  console.log(
    ...
    webRtcPlayerObj && webRtcPlayerObj.video,
    1
  )
...
function playVideo() {
  console.log(
    'Play Videooooooooo we are going to show the play button overlay.'
  )
  console.log(webRtcPlayerObj.video.play())
...
function setupWebRtcPlayer(
  ...
  console.error(
    'webRtcPlayerObj.onWebRtcAnswer',
    webRtcPlayerObj.onWebRtcAnswer
  )
      ...
      if (settingsJSON.PixelStreaming) {
        ...
        if (allowConsoleCommands === false) {
          ...
          if (!document.getElementById('warning-elem-webrtc')) {
            const warningElem1 = document.createElement('p')
          ...
          if (!document.getElementById('warning-elem-encoder')) {
            const warningElem2 = document.createElement('p')
...
function setupStats() {
  ...
  webRtcPlayerObj.onAggregatedStats = (aggregatedStats: {
    ...
    frameWidth: string
    frameHeight: string
    framesDecoded: number | bigint
    packetsLost: number | bigint
    framerate: number | bigint
    framesDropped: number | bigint
    currentRoundTripTime: number
    receiveToCompositeMs: number | bigint
    bitrate: number | bigint
  }) => {
    ...
    const receivedBytes = aggregatedStats.hasOwnProperty('bytesReceived')
  ...
  webRtcPlayerObj.latencyTestTimings.OnAllLatencyTimingsReady =
    function (timings: {
...
let playerElementClientRect: { left: number; top: number } | undefined =
  undefined
let normalizeAndQuantizeUnsigned:
  | ((arg0: number, arg1: number) => any)
  | undefined = undefined
let normalizeAndQuantizeSigned: ((arg0: any, arg1: any) => any) | undefined =
  undefined
...
function createOnScreenKeyboardHelpers(htmlElement: {
  appendChild: (arg0: HTMLButtonElement | HTMLInputElement) => void
}) {
...
function registerMouseEnterAndLeaveEvents(playerElement: {
  ...
  pressMouseButtons: (arg0: any) => void
  onmouseleave: (e: any) => void
  releaseMouseButtons: (arg0: any) => void
}) {
...
function registerHoveringMouseEvents(playerElement: {
  ...
  pressMouseButtons: (e: any) => void
  releaseMouseButtons: (e: any) => void
}) {
...
function registerTouchEvents(playerElement: {
  offsetLeft: number
  offsetTop: number
  ontouchstart: { (e: any): void; (e: any): void }
  onmouseenter: (arg0: any) => void
  ontouchend: { (e: any): void; (e: any): void }
  onmouseleave: (arg0: any) => void
  ontouchmove: { (e: any): void; (e: any): void }
}) {
