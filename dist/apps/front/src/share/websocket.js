...
function socketAdapter(httpPort, streamerPort, streamerServer) {
    ...
    streamerServer.on('connection', function (ws, req) {
        ...
        ws.on('message', function onStreamerMessage(msg) {
            ...
            try {
                ...
                console.error("cannot parse Streamer message: ".concat(msg, "\nError: ").concat(err));
            ...
            if (!player) {
                console.log("dropped message ".concat(msg.type, " as the player ").concat(playerId, " is not found"));
                return;
            }
            if (msg.type === 'answer') {
        ...
        ws.on('close', function (code, reason) {
            console.error("streamer disconnected: ".concat(code, " - ").concat(reason));
    ...
    console.log("WebSocket listening to Players connections on :".concat(httpPort));
    playerServer.on('connection', function (ws, req) {
        ...
        console.log("player ".concat(playerId, " (").concat(req.connection.remoteAddress, ") connected"));
        players.set(playerId, { ws: ws, id: playerId });
        function sendPlayersCount() {
            var playerCountMsg = JSON.stringify({ type: 'playerCount', count: players.size });
        ...
        ws.on('message', function (msg) {
            ...
            try {
                ...
                console.error("Cannot parse player ".concat(playerId, " message: ").concat(err));
            ...
            if (msg.type === 'offer') {
                ...
                console.error("<- player ".concat(playerId, ": unsupported message type: ").concat(msg.type));
        ...
        ws.on('close', function (code, reason) {
            console.log("player ".concat(playerId, " connection closed: ").concat(code, " - ").concat(reason));
            onPlayerDisconnected();
        });
        ws.on('error', function (error) {
            console.error("player ".concat(playerId, " connection error: ").concat(error));
            ws.close(1006 /* abnormal closure */, error);
            onPlayerDisconnected();
        });
