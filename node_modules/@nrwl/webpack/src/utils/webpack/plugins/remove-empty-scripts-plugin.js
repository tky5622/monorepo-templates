...
exports.RemoveEmptyScriptsPlugin = void 0;
...
class RemoveEmptyScriptsPlugin {
    ...
    apply(compiler) {
        ...
        const extensionsWithoutDots = this.options.extensions.map((e) => e[0] === '.' ? e.substring(1) : e);
        const patternOneOfExtensions = extensionsWithoutDots
            .map((ext) => escapeRegExp(ext))
            .join('|');
        const reStylesResource = new RegExp(`[.](${patternOneOfExtensions})([?].*)?$`);
        compiler.hooks.compilation.tap(NAME, (compilation) => {
            ...
            compilation.hooks.chunkAsset.tap(NAME, (chunk, file) => {
                ...
                const resources = customIgnore.length > 0
                    ...
                    : entryResources;
                const isStyleOnly = resources.length &&
...
exports.RemoveEmptyScriptsPlugin = RemoveEmptyScriptsPlugin;
function collectEntryResources(compilation, module, cache) {
    const moduleGraph = compilation.moduleGraph, index = moduleGraph.getPreOrderIndex(module), propNameDependencyId = '__dependencyWebpackRemoveEmptyScriptsUniqueId', resources = [];
